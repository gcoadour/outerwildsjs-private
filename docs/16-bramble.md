...
- ~~**Le son comme signal**~~ — **porté** : le champ audio publie un niveau de
  bruit, calculé sur les sources spatiales effectivement instanciées et leur
  distance à l'auditeur, et les prédateurs l'entendent. Le bruit n'est plus
  déduit des commandes du joueur, c'est-à-dire de son intention.
...
- ~~`CorruptionAnimator`~~ — **porté** : le seuil de découpe suit la fraction de
  boucle et se pose sur les matériaux des ronces, que le répartiteur de shaders
  traite déjà en `alphaCutOff`. Les bornes sont lues dans les champs quand ils
  les portent, 0 → 1 sinon.
